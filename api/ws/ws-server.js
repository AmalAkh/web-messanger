...
{ 
    ...
    let messageDate = `${date.getUTCFullYear()}-${date.getUTCMonth()+1 >= 10 ? date.getUTCMonth()+1: "0"+(date.getUTCMonth()+1)}-${date.getUTCDate() >=10 ? date.getUTCDate():"0"+date.getUTCDate()} ${date.getUTCHours()}:${date.getUTCMinutes() >=10 ? date.getUTCMinutes() : "0"+date.getUTCMinutes()}:${date.getUTCSeconds() >= 10 ? date.getUTCSeconds() : "0"+date.getUTCSeconds()}`; 
    console.log(messageDate);
    await pool.execute("INSERT INTO messages VALUES(?, DEFAULT, ?, ?, ?, DEFAULT )", [messangerRequest.data.text, messangerRequest.data.chatId, senderId, messageId ])
